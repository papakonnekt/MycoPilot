...
## Текущая фаза
**Phase: v3.0.0 stable — public website launch (DONE).** План `plans/done/2026-04-21_feature_landing-website.md` завершён: в репозитории появился статический лендинг `site/` с собственным визуальным направлением, GitHub Pages workflow `.github/workflows/pages.yml`, live URL `https://fockus.github.io/skill-memory-bank/`, а homepage репозитория направлен на сайт. Локальная verification зелёная (`pytest -q tests/pytest` → 119 passed, 14 skipped; `ruff check .` → clean), workflow deploy завершился успешно.

## Завершено 2026-04-21 — Website launch
...
- Текущий remote: `origin=https://github.com/fockus/skill-memory-bank.git`, legacy remote сохранён как `old-origin`
- Рабочее дерево не чистое: во время аудита есть локальные изменения
