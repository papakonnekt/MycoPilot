// =============================================================
// Shared placeholder shell used by every view in Step 1.
// Each view file just supplies `eyebrow`, `title`, `subtitle`,
// `step` and an optional "in this view" hint list.
//
// SOTA lab aesthetic:
//  - Double-Bezel container (outer shell + inner core)
//  - Geist body, Instrument Serif for the big number/section
//  - Eyebrow tag above the title
// =============================================================

import type { ReactNode } from 'react'

export interface ViewPlaceholderProps {
  eyebrow: string
  title: string
  subtitle: string
  step: 'Step 2' | 'Step 3' | 'Future'
  buildHints?: string[]
  children?: ReactNode
}

export function ViewPlaceholder({
  eyebrow,
  title,
  subtitle,
  step,
  buildHints = [],
}: ViewPlaceholderProps) {
  return (
    <div className="bezel-shell">
      <div className="bezel-core p-6 sm:p-10">
        <div className="flex items-center justify-between gap-4">
          <span className="eyebrow-tag">{eyebrow}</span>
          <span className="text-[10px] uppercase tracking-eyebrow text-ink/40">
            {step}
          </span>
        </div>

        <h1 className="mt-6 font-serif text-5xl sm:text-6xl leading-[0.95] tracking-tight text-ink">
          {title}
        </h1>

        <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-graphite-500">
          {subtitle}
        </p>

        <hr className="hairline my-8" />

        {buildHints.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {buildHints.map((hint) => (
              <div
                key={hint}
                className="rounded-2xl bg-black/[0.025] ring-1 ring-black/5 px-4 py-3 text-[13px] text-graphite-600"
              >
                {hint}
              </div>
            ))}
          </div>
        )}

        <div className="mt-10 flex items-center gap-3 text-[11px] uppercase tracking-eyebrow text-ink/40">
          <span className="h-1.5 w-1.5 rounded-full bg-moss-700" />
          <span>Shell ready — content lands in {step.toLowerCase()}</span>
        </div>
      </div>
    </div>
  )
}
